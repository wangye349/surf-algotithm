...
syms x;
syms y;
%I was supposed the change the sigmoid, but it doesn't get the best
%result.So I quit.
% if input_size == 9
%     sig = 1.2;
...
% end
    sig = 1.2;
f = 1/(2 * pi * sig^2) * exp(-((x^2 + y^2)/(2*sig^2)));%The 2 dimentional gassian distribution function. simplified.
a = inline(diff(f,x,2));%this is the second order derivative of x.
b = inline(diff(f,y,2));%this is the second order derivative of y.
c = diff(f,x);
d = inline(diff(c,y));%this is the first order derivative of x, then y.
q = (input_size - 1)/2 + 1;%To get the four parts of hessian matrix.
...
repair = sum(sum(outxx_temp))/(input_size^2);%to solve the problem that all numbles isn't added to zero.
for i = 1:input_size
    for j = 1:input_size
        k = i - q;
        p = j - q;
