function [position] = select(det_img,hessian_matrix_size)
    ...
    for i = side_cant_handle+1:m-side_cant_handle
        for j = side_cant_handle+1:n-side_cant_handle
            if(det_img(i,j)>hessian_matrix_size*(hessian_matrix_size))%3*hessian_matrix_size*hessian_matrix_size*hessian_matrix_size*hessian_matrix_size*hessian_matrix_size)
